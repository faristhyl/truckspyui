...
Starting from [AOT Note](#aot-note) section there is a default [SmartAdmin](https://wrapbootstrap.com/theme/smartadmin-responsive-webapp-WB0573SK0) template's README, which describes all the needed information.

This intro's purpose is to describe additional _Truckspy UI_ project details.

### Test accounts

There are few test accounts used during development, all of them are available in the `login.components.ts` (commented section).

These accounts are valid for both `prod` and `staging`.

### Deployment

Requires write permissions to GCP buckets.

#### Staging

1. Run `npm run-script build-staging` from the project root `$PROJECT_ROOT`
1. Deploy generated sources (located in the `$PROJECT_ROOT/dist` directory) to the GCP bucket named `app.truckspyapi.com`
1. Set `Cache-Control: no-cache, max-age=0` for `index.html` file in the bucket

All these steps are scripted in the `deploy-stage` section of `$PROJECT_ROOT/Makefile`.

#### Prod

1. Run `npm run-script build-prod` from the project root `$PROJECT_ROOT`
1. Deploy generated sources (located in the `$PROJECT_ROOT/dist` directory) to the GCP bucket named `app.truckspy.io`
1. Set `Cache-Control: no-cache, max-age=0` for `index.html` file in the bucket

All these steps are scripted in the `deploy-prod` section of `$PROJECT_ROOT/Makefile`.

### Important things to know during development

There are functionality need to be actual, either part of the SmartAdmin template or part of the project:

1. __Login As__ admin's functionality (project specific)
1. __Voice Commands__, check `voice_commands` from `smartadmin.config.ts` and related logic (template specific)
1. __Keyboard Shortcuts__, check `shortcuts.component.ts` and related logic (template specific)
