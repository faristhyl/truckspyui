...
export class DraftComponent implements OnInit, OnDestroy {
   ...
   * Send logic.
   */
  send(message: Message, actionComponent: LongActionLinkComponent) {
    this.restService.sendMessage(message)
      .subscribe(
        success => {
  ...
  /**
   * Continue writing draft message.
   */
  openDraftMessage(message: Message) {
    this.messagesService.storeDraftMessage(message);
  ...
  /**
   * Constructor to instantiate an instance of DraftComponent.
   */
  constructor(
  ...
  loadMessages(callback: Function = null) {
    let params = new FilterParams(this.page, `${this.sort}.${this.ascending ? 'ASC' : 'DESC'}`);
    this.restService.getDraftMessages(params, this.fixedQuery, this.driverId)
      .subscribe(
        data => {
